 ...
 * \mainpage
 * \section Intro Intro
 * This program is for the 4-In-A-Row Soldering Kit
 * \author Electro707(Jamal Bouajjaj)
 * \copyright MIT License
 * 
 * \section main_f Main File
 * Checkout main.c for all of the program.
 * 
 * \section int_update Interrupt Update
 * The shift register (and thus the LEDs) are updated from an internal timer. This timer interrupts once
 * every 128 uS ((1/16Mhz)*8*256). 
 * When the interrupt is ran, the LEDs are updated one color at a time (red, green, then blue) per row defined by the RGB mask
 * variable mask_test. This is needed as if a red and green LED are on at the same time, the green LED will never turn on due to
 * the red LED's lower foward voltage.
 * After all 3 colors are updated per row, the next row is selected (the cathodes for that row is pulled low). This keeps on running
 * until the last row is done, in which the process is repeated again.
 * With a nice trick of the human eye called presistance of vision, this is enough to trick us into thinking all of the LEDs
 ...
 */
uint32_t led[6];                /**< The shift register's output bits per scan*/
uint8_t current_row = 0;        /**< The current row from 0-5 */
uint32_t current_led = 0;       /**< The current output mask 0-2 (See ISR(TIMER0_OVF_vect) for more details)*/
uint8_t status_led_color = 0;   /**< The status's LED color */
/**
 ...
 */
uint16_t players[6];        
uint8_t active_player = 0; /**< Stores the active player. 1 for player 1, 2 for player 2. \todo: have this be of an enum type */
/**
...
int main(void)
{
	...
	PORTB &= ~(1<<0);	// Enable the status LED's cathode
	
	SPI_ENABLE;	
	sei();			// Turn on interrupts
	ENABLE_595_OUTPUT;	// Enable the 595's output driver

 	clearBuffer(players, 6);		// Clear the player's buffer for a fresh start
    ...
    while (1){ // The game loop!
        ...
        if(c == 7){             // If we press the button on the driver board which resets the game
            end_of_game(0);
            continue;
        }
        ...
        if(((players[0] >> (c*2)) & 0b11) == 0){		// Only switch player and find the row if the column isn't fully occupied
            ...
            uint8_t w = checkForWinner();	// Check for who, or none, player that wone
            if(w != 0){
            	end_of_game(w);
            	continue;
        	}
